...
function analyzeCommonPaths(urls: string[]): { path: string; count: number }[] {
  ...
  const pathEntries = Object.entries(pathCounts).map(([path, count]) => ({ path, count }));
  
  // Sort by count (descending)
  return pathEntries.sort((a, b) => b.count - a.count);
}

/**
 * Get SEO recommendations based on crawl data
 */
function getSeoRecommendations(
  ...
  // Add broken links recommendation if needed
  if (brokenLinksCount > 0) {
    recommendations.push({
      title: 'Исправьте битые ссылки',
      description: `Обнаружено ${brokenLinksCount} битых ссылок. Исправьте их для улучшения SEO и пользовательского опыта.`,
      priority: brokenLinksCount > 10 ? 'high' : 'medium'
    });
  }
  
  // Add duplicates recommendation if needed
  if (duplicatesCount > 0) {
    recommendations.push({
      title: 'Устраните дубликаты страниц',
      description: `Найдено ${duplicatesCount} групп дублированных страниц. Используйте каноничные URL и редиректы для устранения проблемы.`,
      priority: duplicatesCount > 5 ? 'high' : 'medium'
    });
  }
  
  // Check site depth
  const deepPages = depthData.filter(d => d.level > 3).reduce((sum, item) => sum + item.count, 0);
  if (deepPages > 0 && totalUrls > 0 && (deepPages / totalUrls > 0.3)) {
    recommendations.push({
      title: 'Оптимизируйте структуру сайта',
      description: `${Math.round((deepPages / totalUrls) * 100)}% страниц имеют глубину вложенности более 3 уровней. Оптимизируйте структуру для лучшей индексации.`,
      priority: (deepPages / totalUrls > 0.5) ? 'high' : 'medium'
    });
  }
  
  // Add general recommendations
  recommendations.push({
    title: 'Обновите карту сайта',
    description: 'Создайте или обновите карту сайта (sitemap.xml) и добавьте ее в Google Search Console для улучшения индексации.',
    priority: 'medium'
  });
  
  recommendations.push({
    title: 'Проверьте robots.txt',
    description: 'Убедитесь, что файл robots.txt настроен правильно и не блокирует важный контент от индексации.',
    priority: 'medium'
  });
  
  recommendations.push({
    title: 'Оптимизируйте структуру URL',
    description: 'Используйте семантические URL, которые включают ключевые слова и понятны для пользователей и поисковых систем.',
    priority: 'low'
  });
  
  recommendations.push({
    title: 'Проверьте скорость загрузки',
    description: 'Оптимизируйте скорость загрузки страниц для улучшения пользовательского опыта и SEO-рейтинга.',
    priority: 'medium'
  });
  
  recommendations.push({
    title: 'Настройте отслеживание аналитики',
    description: 'Убедитесь, что инструменты аналитики настроены правильно для отслеживания посещений и поведения пользователей.',
    priority: 'low'
  });
...
/**
 * Get recommendation priority text
 */
...
/**
 * Apply recommendation priority color to jsPDF document
 */
function applyRecommendationPriorityColor(doc: jsPDF, priority: 'high' | 'medium' | 'low'): void {
  const color = getRecommendationPriorityColor(priority);
  doc.setTextColor(color[0], color[1], color[2]);
}

/**
 * Get site summary text based on metrics
 */
function getSiteSummary(
  ...
  if (score >= 90) {
    summary = `Ваш сайт находится в отличном состоянии с общим показателем ${score}/100. `;
  } else if (score >= 70) {
    summary = `Ваш сайт в хорошем состоянии с общим показателем ${score}/100, но есть возможности для улучшения. `;
  } else if (score >= 50) {
    summary = `Ваш сайт требует оптимизации (общий показатель: ${score}/100). `;
  } else {
    summary = `Ваш сайт нуждается в значительной оптимизации (общий показатель: ${score}/100). `;
  }
  ...
  if (duplicatesCount > 0) {
    summary += `Найдено ${duplicatesCount} групп дублированного контента, которые могут негативно влиять на рейтинг. `;
  } else {
    summary += 'Дубликатов страниц не найдено, что положительно влияет на SEO. ';
  }
  
  summary += `Всего проанализировано ${totalUrls} URL-адресов. Рекомендуется следовать предложенным рекомендациям для улучшения SEO-производительности сайта.`;
