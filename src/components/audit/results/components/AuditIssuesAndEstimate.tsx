    ...
    <div className="neo-card p-6 mt-8">
      ...
      <div className="mb-6">
        ...
        {hasErrors ? (
          ...
          <div className="text-green-600">
            Ошибки не найдены — сайт соответствует базовым требованиям SEO.
          </div>
        )}
      </div>
      {typeof optimizationCost === "number" && (
        <div>
          ...
          <div className="mb-2">
            Итоговая стоимость оптимизации:{" "}
            <span className="font-bold text-primary">{optimizationCost} ₽</span>
          </div>
              ...
              <tbody>
                {optimizationItems.map((item, i) => (
                  <tr key={i} className="border-t">
                    ...
                    <td className="py-1 px-2">{item.count || 1}</td>
                    <td className="py-1 px-2">{item.totalPrice || item.price || 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="text-xs text-muted-foreground">
            * Стоимость может отличаться в зависимости от индивидуальных требований
          </div>
        </div>
      )}
    </div>
