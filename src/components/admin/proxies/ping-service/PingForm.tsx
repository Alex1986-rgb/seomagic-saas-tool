    ...
    <div className="space-y-6">
      ...
      {!isLoading && (
        <Alert>
          ...
          <AlertDescription>
            {/* Раньше здесь советовали «собрать новые прокси» при проблемах с пингом.
                Прокси в пинге не участвуют: запрос уходит из браузера напрямую. */}
            Пинг отправляется из браузера напрямую: переключатели прокси на запрос не влияют,
            браузер не умеет слать запросы через прокси. Многие пинг-сервисы не принимают
            запросы из браузера (CORS) — тогда в результатах будет ошибка, а не успех.
          </AlertDescription>
        </Alert>
      )}
    </div>
