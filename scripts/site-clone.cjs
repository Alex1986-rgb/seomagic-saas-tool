...
function contentBlockV2(topic, html, pageUrl) {
  ...
  const itemsLine = items.length
    ? H('Что представлено') + `<p>В этом разделе, в частности: <strong>${items.map(esc).join(', ')}</strong> — и другие позиции. Подберём вариант под ваши требования; смежные решения смотрите также в ${nextL()}.</p>`
    : '';
  ...
  const usecase = H('Кому и для чего подойдёт') + `<p>Этот раздел универсален: подходит и для дома, и для бизнеса. Для частного использования важны комфорт и внешний вид, для коммерческих помещений — износостойкость и практичность. Подскажем вариант под ваш сценарий и поможем собрать решение целиком.</p>`;
